...
const PRESET_LOGO_PRO = `生成一张 16:9 横向博客文章封面，主题为{{ARTICLE_TITLE}}。

文章概要：
{{ARTICLE_OUTLINE}}

请根据文章标题与概要，自行决策以下三项内容（不要求固定，贴合文章主题即可）：
1. 背景关键词：提炼最核心的 1 个中文关键词，作为背景超大文字；
2. 配色方案：依主题气质选择一组柔和的高级配色（如科技蓝、暖橙、墨绿、紫罗兰等），并说明背景渐变基调；
3. 中央视觉主体：设计一个与主题含义强相关的简洁 3D 图标组合，作为画面唯一焦点。

整体视觉风格：
极简、现代、轻 3D、科技感、高级感，参考优秀开发者博客的现代技术封面。
使用柔和的渐变背景（颜色由你决策，倾向浅色系），干净留白，柔和环境光，细腻阴影，轻微玻璃质感和反射效果，圆角造型，避免复杂装饰。

核心构图：
严格采用「大字 + 中央主体遮挡」构图。
画面中央横向放置超大的中文关键词「{{KEYWORD}}」，作为背景文字。
文字尺寸非常大，横跨画面左右两侧，颜色为比背景稍深的半透明色。
中央主体必须位于文字前方，并明显遮挡「{{KEYWORD}}」中间部分。
形成清晰的：
「文字 [中央主体] 文字」
空间层级。
两侧文字自然露出，不要求完整显示。
...
主体必须是整个画面的视觉焦点，层数与元素从简，整体不超过 3 层。

不要表现复杂代码、复杂数据库结构或完整 UI。
...
不要额外装饰性文字。
不要把 Logo 和关键词并排放置。
必须保证「文字在后，主体在前」，中央主体明显遮挡背景关键词。

最终效果：
类似「XX [大型 3D 主题主体] XX」的构图，
中央主体突出，背景文字巨大但弱化，
整体简洁、柔和、现代、具有技术博客封面的高级感。`;
...
export const OFFICIAL_COVER_TEMPLATES: CoverTemplate[] = [
  ...
  {
    id: "preset_logo_pro",
    name: "Logo Pro（AI 决策配色与主体）",
    description: "大字+中央主体遮挡构图，AI 依文章主题自选关键词、配色与 3D 主体，轻 3D 高级感",
    template: PRESET_LOGO_PRO,
    builtIn: true,
  },
  {
